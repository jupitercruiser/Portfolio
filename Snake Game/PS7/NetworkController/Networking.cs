// Author: Brandy Cervantes & Mia Mellem 2023
using System;
...
public static class Networking
{
    ...
    //  variable that holds a list of clients
    // NOMADIC FIELDS
    ...
    private static void AcceptNewClient(IAsyncResult ar)
    {
        ...
        { 
            ...
            socketState.OnNetworkAction = toCall;
        ...
        {
            SocketState newSocketState = new SocketState(toCall, "Connection unsuccessful!");
    ...
    public static void ConnectToServer(Action<SocketState> toCall, string hostName, int port)
    {
        ...
        try
        {
            ...
            if (!foundIPV4)
            {
                ...
                SocketState socketState = new SocketState(toCall, "IP Address not found!");
                socketState.OnNetworkAction(socketState);
            }
        }
        catch (Exception)
        {
            ...
            {
                // TODO: Indicate an error to the user, as specified in the documentation
                SocketState socketState = new SocketState(toCall, "Invalid host name!");
                socketState.OnNetworkAction(socketState);
            }
        }
        ...
        try
        {
            ...
            Tuple<SocketState, Action<SocketState>> tup = new Tuple<SocketState, Action<SocketState>>(state, toCall);

            IAsyncResult result = state.TheSocket.BeginConnect(ipAddress, port, ConnectedCallback, tup);

            bool success = result.AsyncWaitHandle.WaitOne(3000, true);
            if (success == false)
            {
                state = new SocketState(toCall, "Connection timed out!");
        ...
        {
            state = new SocketState(toCall, "Connection was unsuccessful!");
            state.TheSocket.Close();
            state.OnNetworkAction(state);
        }
    }
    ...
    {
        Tuple<SocketState, Action<SocketState>> tup = (Tuple<SocketState, Action<SocketState>>)ar.AsyncState!;

        SocketState socketState = tup.Item1;
        Action<SocketState> toCall = tup.Item2;
        ...
        try
        {
            s.EndConnect(ar); // Error can happen here
            socketState.OnNetworkAction(socketState);
        }
        catch(Exception)
        {
            socketState = new SocketState(toCall, "Connection unsuccessful!");
            socketState.OnNetworkAction(socketState);
        }
    }
    ...
    /// GetData should not continue the receive loop.
    public static void GetData(SocketState state)
    {
        try
        {
            Tuple<SocketState, Action<SocketState>> tup = new Tuple<SocketState, Action<SocketState>>(state, state.OnNetworkAction);
            state.TheSocket.BeginReceive(state.buffer, 0, state.buffer.Length, SocketFlags.None, ReceiveCallback, tup);
        }
        catch(Exception)
        {
            state = new SocketState(state.OnNetworkAction, state.TheSocket);
            state.OnNetworkAction(state);
        }
    }
    ...
    private static void ReceiveCallback(IAsyncResult ar)
    {
        ...
        {
            socketState = new SocketState(toCall, "Unable to receive data!");
            socketState.OnNetworkAction(socketState);
        }
    }
    ...
    /// since there is no SocketState involved, just a Socket.
